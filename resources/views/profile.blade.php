<x-layouts.student title="Profil Pilot" active="profile">
    <div class="max-w-6xl mx-auto space-y-12">
        {{-- Profile Hero Card --}}
        <div class="bg-white rounded-2xl md:rounded-[3rem] shadow-xl overflow-hidden border-4 border-slate-50 relative">
            ...
            <div class="p-6 md:p-12 flex flex-col md:flex-row items-center gap-8 md:gap-12">
                <div class="relative">
                    <div class="size-40 md:size-64 rounded-full bg-blue-50 border-8 border-white shadow-lg p-4 overflow-hidden">
                ...
                <div class="flex-1 space-y-8 text-center md:text-left">
                    <div>
                        <h3 class="text-3xl md:text-5xl font-black text-slate-800 mb-4">{{ auth()->user()->name }}</h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="bg-slate-50 p-4 md:p-6 rounded-2xl md:rounded-3xl border-2 border-slate-100 flex items-center gap-4">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
                @foreach($badges as $badge)
                    ...
                    @if($isUnlocked)
                        ...
                        <div class="bg-white p-4 md:p-6 rounded-2xl md:rounded-[2rem] border-4 border-slate-50 flex flex-col items-center text-center gap-3 md:gap-4 card-hover shadow-sm group">
                            <div class="relative size-16 md:size-24 rounded-full {{ $themeWrapper }} flex items-center justify-center mb-1 md:mb-2">
                                ...
                                <span class="material-symbols-outlined text-2xl md:text-4xl {{ $iconColor }} font-fill-1">{{ $badge->icon }}</span>
                            </div>
                            <div>
                    ...
                    @else
                        {{-- Locked Badge Option --}}
                        <div class="bg-slate-50 p-4 md:p-6 rounded-2xl md:rounded-[2rem] border-4 border-slate-100 flex flex-col items-center text-center gap-3 md:gap-4 opacity-40 group filter grayscale">
                            <div class="size-16 md:size-24 rounded-full bg-slate-200 flex items-center justify-center mb-1 md:mb-2">
                                <span class="material-symbols-outlined text-2xl md:text-4xl text-slate-400 font-fill-1">{{ $badge->icon }}</span>
                            </div>
                            <div>
